...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-sm">
      ...
      <p className="text-blue-600">{formatINR(payload[0]?.value)}</p>
...
export function WeekdayChart({ data = [], loading = false }) {
  ...
  if (!data.length) return <div className="h-52 flex items-center justify-center text-gray-400">No data</div>;
      ...
      <BarChart data={data} margin={{ top: 5, right: 5, left: 5, bottom: 0 }}>
        ...
        <Bar dataKey="revenue" radius={[4, 4, 0, 0]}>
          {data.map((entry, i) => (
            <Cell
              key={i}
              fill={entry.revenue === maxRevenue ? "#3b82f6" : entry.revenue > maxRevenue * 0.75 ? "#60a5fa" : "#bfdbfe"}
            />
          ))}
        </Bar>
      </BarChart>
